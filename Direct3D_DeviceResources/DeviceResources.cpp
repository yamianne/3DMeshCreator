...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        CD3D11_DEPTH_STENCIL_DESC nodepthState(
            FALSE,
        ...
        ThrowIfFailed(m_d3dDevice->CreateDepthStencilState(&nodepthState, m_noDepthWriteState.GetAddressOf()));
...
void DeviceResources::UpdateConstantBuffer(int width, int height)
{
    m_cbWindowAspectRatio.SetData(m_d3dContext.Get(), { height * 1.0f / width, 1.0f, 0.0f, 1.0f });
    auto buffer = m_cbWindowAspectRatio.GetBuffer();
    m_d3dContext->GSSetConstantBuffers(0, 1, &buffer);

    float aspectRatio = (float)width / height;
    auto projectionMatrix = MatrixTransformations::CreateProjectionMatrix(XM_PI / 4.0f, aspectRatio, 0.1f, 1000.f);

    m_cbProjectionMatrix.SetData(m_d3dContext.Get(), projectionMatrix);
    buffer = m_cbProjectionMatrix.GetBuffer();
